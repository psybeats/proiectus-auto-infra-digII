...
def crear_db():
    try:
        ...
        connection = pymysql.Connection(host="localhost", user="root", password="root", db="mysql")
        cursorr = connection.cursor()
        # Se crea una nueva DB utilizando la conexión existente
        cursorr.execute("CREATE DATABASE IF NOT EXISTS clinicaDental")
        # Finaliza la conexión
        connection.close()
        cursorr.close()
        # Se crea una nueva conexión utilizando la DB creada anteriormente
        connection = pymysql.Connection(host="localhost", user="root", password="root", db="clinicaDental")
        cursorr = connection.cursor()
